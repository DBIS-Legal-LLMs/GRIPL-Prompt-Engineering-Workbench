...
class BpmnAnalyzer(
    ...
    private val prompt: AnalysisPrompt
) {
    ...
    private val activitiesOnly = prompt.classificationScope == ClassificationScope.ACTIVITIES_ONLY

    /**
     * Analyzes the supplied BPMN model and returns the elements classified as
     * GDPR-relevant.
     *
     * Baseline analysis uses the raw BPMN XML and ignores RAG. Regular analysis
     * uses extracted BPMN elements and optionally enriches them with RAG context.
     */
    fun analyzeBpmnForGdpr(
        ...
        return if (prompt.isBaseline) {
            analyzeBaseline(sessionId, bpmnXml, bpmnElements)
        } else {
            analyzePromptEngineering(sessionId, bpmnElements, useRag, ragMode)
        }
    }

    /**
     * Analyzes the BPMN model using the baseline processing behavior of the
     * former /gdpr/analysis/baseline endpoint.
     *
     * The baseline prompt now is supplied through the prompt library, but the
     * BPMN input remains raw XML and no RAG context is retrieved.
     */
    private fun analyzeBaseline(
        sessionId: String,
        bpmnXml: String,
        bpmnElements: Set<BpmnElement>
    ): AnalysisResponse {
        val systemPrompt = PromptTemplateRenderer.render(prompt.template, prompt.variables, useRag = false)

        val aiService = BpmnAnalysisAiServiceFactory.create(llm, memoryProvider, systemPrompt)

        val result = safetyNet.safeGuardAnalysisResultParsing(
            sessionId = sessionId,
            maxRetries = 3
        ) {
            aiService.analyzeXml(sessionId, bpmnXml)
        }
    ...
    /**
     * Analyzes the BPMN model using the specified prompt version from the prompt
     * library that is not the baseline version. 
     *
     * Without RAG, the extracted BPMN elements are analyzed directly. With RAG,
     * retrieved GDPR context is added to the model input.
     * 
     * This is equivalent to the behavior of the former /gdpr/analysis/prompt-engineering endpoint.
     */
    private fun analyzePromptEngineering(
        sessionId: String,
        bpmnElements: Set<BpmnElement>,
        useRag: Boolean,
        ragMode: RagMode
    ): AnalysisResponse {
        val systemPrompt = PromptTemplateRenderer.render(prompt.template, prompt.variables, useRag)

        val bpmnAnalysisAiService = BpmnAnalysisAiServiceFactory.create(llm, memoryProvider, systemPrompt)
        ...
        if (useRag) {
            ...
            val result = safetyNet.safeGuardAnalysisResultParsing(sessionId, maxRetries = 3) {
                ...
                bpmnAnalysisAiService.analyzeElementsWithRagContext(sessionId, formattedPrompt)
            }
            ...
            val result = safetyNet.safeGuardAnalysisResultParsing(sessionId, maxRetries = 3) {
                bpmnAnalysisAiService.analyzeElements(sessionId, bpmnElements)
            }
